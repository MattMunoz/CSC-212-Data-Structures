...
namespace main_savitch_2C
{
    ...
    public:
        ...
        void next(double r);
        void reset() { count = 0; total = 0; };
        // CONSTANT MEMBER FUNCTIONS
        int length() const { return count; };
        double sum() const { return total; };
        double mean() const { assert(count > 0); return total / length(); };
        double minimum() const { assert(count > 0); return tinyest; };
        double maximum() const { assert(count > 0); return largest; };
        // FRIEND FUNCTIONS
